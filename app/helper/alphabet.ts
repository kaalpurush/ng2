export class Alphabet {
	static POSITION = { SELF: 0, ISOLATED: 1, END: 2, MIDDLE: 3, START: 4 };
	...
	static get(index, position: number) {
		if (typeof this.alphabets[index - 1][position] === 'undefined')
			return this.alphabets[index - 1][2];
		return this.alphabets[index - 1][position];
	}

	static getAll(position: number): Array<any> {
		return this.alphabets.map((v) => v.slice(position, position + 1).reduce((v) => v));
	}

	static getLetter(index, position: number) {
		return this.get(index, position).letter;
	}

	static getAllLetters(position: number) {
		return this.getAll(position).map((v) => v.letter);
	}

	static getPositions(index) {
		return this.alphabets[index - 1];
	}

	static getIndexForCode(code: string) {
		let result = null;
		this.alphabets.every((f, i) => {
			f.every(s=> {
				if (s.code == code) {
					result = i;
					return false;
	...
	static getRandomExcept(count: number, index: Array<number>) {
		let alphabets = this.alphabets.filter((v, i) => index.indexOf(i) < 0);
		let random = this.randomDistinct(count, alphabets.length);

		return alphabets.filter((v, i) => {
	...
	static randomDistinct(count: number, to: number = 20, from: number = 0) {
		let random = [];
		if (to - from < count)
			return null;
